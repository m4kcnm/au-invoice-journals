...
AI_SYSTEM_PROMPT = """You are an Australian Tax Invoice and Accounts Payable AI Auditor.
Extract structured bookkeeping attributes from this Australian tax invoice text.

GST rules in Australia:
- Standard GST is 10% (1/11th of GST-inclusive amount).
- Water supply, council rates, and financial charges are GST-free.
- Telco, internet, energy, software, and general supplies are standard taxable (10% GST).

Return ONLY valid JSON matching this schema:
{
  "supplier_name": "Full legal or trading business name (e.g. Flip, Origin Energy, Telstra)",
  "supplier_abn": "11 digit ABN without spaces or null",
...
def render_pdf_images(path: Path, max_pages: int = 2) -> list[Image.Image]:
    try:
        doc = fitz.open(path)
        images = []
...
def capture_document(path: Path) -> dict[str, Any]:
    ...
    if len(text) < 40:
        ...
        if ocr:
            text = (text + "\n" + ocr).strip()
    return {"text": text, "images": images, "ocr": ocr}
...
def run_hybrid_extract(text: str, ollama_url: str, model: str | None = None) -> tuple[dict[str, Any], str]:
    abns = find_abn_candidates(text)
    ...
    hint_str = f"\n[OCR Context Hints: {'; '.join(hints)}]" if hints else ""
    ...
    chosen_model = model or pick_model(available, prefer_vision=False) or "llama3.2:latest"
    ...
    try:
        ...
        if data and isinstance(data, dict):
            method = f"AI ({chosen_model}) + Regex Validation"
    except Exception:
        data = None
