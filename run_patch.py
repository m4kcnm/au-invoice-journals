...
print("Applying accounting safeguards & audit events...")
...
if '"requires_review"' not in gst_code:
    ...
    if split_target not in gst_code:
        fail("split_target 'if treatment == \"gst_free\":' not found in app/gst.py")
...
# 3. Update app/journals.py
if "Posting is blocked: GST treatment on" not in journals_code:
    target_loop = 'for line in invoice.lines:\n        mapped = resolve_mapping(session, invoice, line)'
    if target_loop not in journals_code:
        fail("target_loop for invoice lines not found in app/journals.py")
    
    # We detect requires_review inside the loop and append a blocking warning
    loop_with_gst_check = """for line in invoice.lines:
        mapped = resolve_mapping(session, invoice, line)
    ...
    journals_code = journals_code.replace(target_loop, loop_with_gst_check, 1)

# 4. Update app/web.py (Audit events)
...
# 5. Clean app/static/app.js & templates/invoices.html
...
print("✓ All application source files updated successfully.")

for f in ["app/gst.py", "app/extract.py", "app/journals.py", "app/web.py"]:
    py_compile.compile(f, doraise=True)
print("✓ Byte compilation passed cleanly across all modules.")
